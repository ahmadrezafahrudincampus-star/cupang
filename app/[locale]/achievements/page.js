...
export async function generateMetadata({ params }) {
  ...
  return {
    title: locale === 'en' ? 'Show Records & Contest Placements' : 'Catatan Prestasi & Kontes',
    alternates: {
      canonical: `${baseUrl}/${locale}/achievements`,
      languages: { 'id': `${baseUrl}/id/achievements`, 'en': `${baseUrl}/en/achievements` }
    }
  };
}
...
export default async function PolicyPage({ params }) {
  const { locale } = await params;
  const title = locale === 'en' ? 'Show Records & Contest Placements' : 'Catatan Prestasi & Kontes';
