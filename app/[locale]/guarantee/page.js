...
export async function generateMetadata({ params }) {
  ...
  return {
    title: locale === 'en' ? 'Live Arrival Guarantee (DOA)' : 'Garansi Ikan Hidup (DOA)',
    alternates: {
      canonical: `${baseUrl}/${locale}/guarantee`,
      languages: { 'id': `${baseUrl}/id/guarantee`, 'en': `${baseUrl}/en/guarantee` }
    }
  };
}
...
export default async function PolicyPage({ params }) {
  const { locale } = await params;
  const title = locale === 'en' ? 'Live Arrival Guarantee (DOA)' : 'Garansi Ikan Hidup (DOA)';
