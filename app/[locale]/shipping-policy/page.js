...
export async function generateMetadata({ params }) {
  ...
  return {
    title: locale === 'en' ? 'Shipping & Quarantine Policy' : 'Kebijakan Pengiriman & Karantina',
    alternates: {
      canonical: `${baseUrl}/${locale}/shipping-policy`,
      languages: { 'id': `${baseUrl}/id/shipping-policy`, 'en': `${baseUrl}/en/shipping-policy` }
    }
  };
}
...
export default async function PolicyPage({ params }) {
  const { locale } = await params;
  const title = locale === 'en' ? 'Shipping & Quarantine Policy' : 'Kebijakan Pengiriman & Karantina';
