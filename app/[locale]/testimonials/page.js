...
export async function generateMetadata({ params }) {
  ...
  return {
    title: locale === 'en' ? 'Collector Proof & Testimonials' : 'Ulasan & Testimoni Kolektor',
    alternates: {
      canonical: `${baseUrl}/${locale}/testimonials`,
      languages: { 'id': `${baseUrl}/id/testimonials`, 'en': `${baseUrl}/en/testimonials` }
    }
  };
}
...
export default async function PolicyPage({ params }) {
  const { locale } = await params;
  const title = locale === 'en' ? 'Collector Proof & Testimonials' : 'Ulasan & Testimoni Kolektor';
