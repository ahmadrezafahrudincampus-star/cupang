...
export default function EditJournalPage() {
  ...
  useEffect(() => {
    ...
    async function loadPost() {
      ...
      const { data: post } = await supabase.from('journal_posts').select('*').eq('id', id).maybeSingle()
  ...
  return (
    <div className="max-w-4xl space-y-6">
      <div className="flex justify-between items-center border-b border-white/[0.08] pb-4">
        <div>
          <h1 className="text-3xl font-display text-primary">Edit Artikel: {form.title}</h1>
          <p className="text-xs text-on-surface-variant font-body mt-1">Slug: {form.slug}</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6 bg-surface p-8 border border-white/[0.08] rounded-xl">
        ...
        <div className="pt-2 border-t border-white/[0.08]">
          <label className="flex items-center gap-2 text-sm text-on-surface cursor-pointer">
            ...
            />
            Publikasikan Artikel ke Website
          </label>
        </div>
