...
export default async function AdminLayout({ children }) {
  ...
  const userEmail = user?.email || (devSession ? 'superadmin@aquaticart.com' : 'admin@aquaticart.com')
        ...
        <header className="h-14 bg-surface border-b border-white/[0.08] flex items-center justify-between px-8 shrink-0">
          ...
          <div className="flex items-center space-x-6">
            ...
            <form action="/api/auth/signout" method="post">
              <button type="submit" className="text-xs uppercase tracking-widest text-red-400 hover:text-red-300 transition-colors font-semibold">
                Keluar
              </button>
            </form>
          </div>
